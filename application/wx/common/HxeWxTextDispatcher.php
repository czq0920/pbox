...
class HxeWxTextDispatcher{
			...
			switch ($v['functionType']){
                ...
                case self::$FUNC_TYPE_SPLIT_ARGS:
					...
					$arrs = array_filter(explode($v['splitChar'], $content),function($arr){
						return $arr != HxeWxTextDispatcher::$CURRENT_SPLIT_CHAR;
					});
                ...
                case self::$FUNC_TYPE_FIXED:
                    if (in_array(strtolower($content), $v['functionName'])) {
                        switch ($v['replyType']){
                            case 'text' :
                                HxeWx::fetchTextResult($v['replyContent']['content']);
                                break;
                            case 'news':
                                HxeWx::fetchNewsResult($v['replyContent']['newsArr']);
                                break;
                            case 'music':
                                HxeWx::fetchMusicResult($v['replyContent']['title'],
	...
	static function setConfig($configArray = array()){
		if (!$configArray) 
			$configArray = require_once APP_PATH. MODULE_NAME .'/Conf/TextDispatcherConfig.php';
