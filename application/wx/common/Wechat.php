...
class Wechat
{
	...
	{
		$this->token = isset($options['token'])?$options['token']:config('$options')['token'];
		$this->encodingAesKey = isset($options['encodingaeskey'])?$options['encodingaeskey']:config('$options')['encodingaeskey'];
		$this->appid = isset($options['appid'])?$options['appid']:config('$options')['appid'];
		$this->appsecret = isset($options['appsecret'])?$options['appsecret']:config('$options')['appsecret'];
    ...
    public function log($log){
		if ($this->debug) {
			if (function_exists($this->logcallback)) {
				...
				Log::init([
					'type' => 'File',
					'path' => APP_PATH.'wx_logs/'
				]);
				Log::write('微信记录日志：'.$log, Log::DEBUG);
				return true;
			}
		}
		return false;
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	protected function setCache($cachename,$value,$expired=7200){
		...
		if(Cache::set($cachename,$value,$expired)){
			return  true;
		}
		return false;
	}
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
		...
		$authname = 'WX_API_ACCESSTOKEN'.$appid;
	 ...
	 */
	public function createMenu($data,$type=0){
		//0代表json格式菜单，1为数组格式菜单
		if (!$this->access_token && !$this->checkAuth()) return false;
		$data = $type==0?$data:self::json_encode($data);
		$result = $this->http_post($aaa = self::API_URL_PREFIX.self::MENU_CREATE_URL.'access_token='.$this->access_token,$data);
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
